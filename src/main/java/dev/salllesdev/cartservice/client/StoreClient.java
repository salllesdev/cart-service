package dev.salllesdev.cartservice.client;

import dev.salllesdev.cartservice.DTO.ClientProductResponse;
import dev.salllesdev.cartservice.exceptions.CustomErrorDecoder;
import org.springframework.cloud.openfeign.FeignClient;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;

import java.util.List;

@FeignClient(name = "FakeStoreAPI", url = "${products.client.API}", configuration = CustomErrorDecoder.class)
public interface StoreClient {

    @GetMapping("products/")
    List<ClientProductResponse> getAllProducts();

    @GetMapping("products/{id}")
    ClientProductResponse getProductById(@PathVariable Long id);

}
